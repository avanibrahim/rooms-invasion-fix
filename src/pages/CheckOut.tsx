...
const handleSuccessOK = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    country: "Indonesia",
    // Payment
    paymentMethod: "qris", // default ke QRIS (validasi step 2 aman)
    cardNumber: "",
    expiryDate: "",
    cvv: "",
    cardName: "",
  ...
  const [shippingOption, setShippingOption] = useState("pickup"); // 'pickup' | 'flat'
  const shippingCost = shippingOption === "flat" ? 20000 : 0;

  const tax = 0; // jika belum pakai pajak
  ...
  const buildItemsList = () =>
    items
      .map(
        (item) =>
          `• ${item.name} (${item.size}, ${item.color}) x${item.quantity} - Rp ${(item.price * item.quantity).toLocaleString(
            "id-ID"
          )}`
      )
      .join("\n");

  const formatOrderMessage = (orderNumber, itemsList) => {
    return `*PESANAN BARU*
...
*Metode Pembayaran:* ${paymentMethodText(formData.paymentMethod)}
...
Mohon konfirmasi pesanan ini. Terima kasih!`;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Cek stok
      const outOfStock = items.filter((it) => it.stock === 0);
      ...
      // Nomor order + rangkai payload
      const orderNumber = generateOrderNumber();
      const itemsList = buildItemsList();
      const waMessage = formatOrderMessage(orderNumber, itemsList);
      ...
      const orderPayload = {
        ...
        shippingMethodText,
        paymentMethodText: paymentMethodText(formData.paymentMethod),
              ...
              <div key={stepItem.number} className="flex items-center flex-shrink-0">
                <div
                  className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-semibold text-sm sm:text-base ${
                    step >= stepItem.number ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-600"
                  }`}
                >
                  {stepItem.number}
                </div>
                <span
                  className={`ml-2 font-medium text-xs sm:text-base ${step >= stepItem.number ? "text-gray-900" : "text-gray-500"}`}
                >
                  <span className="sm:hidden">{stepItem.title}</span>
                  <span className="hidden sm:inline">{stepItem.fullTitle}</span>
                </span>
                {index < 2 && <div className={`w-8 sm:w-16 h-0.5 ml-2 sm:ml-4 ${step > stepItem.number ? "bg-gray-900" : "bg-gray-200"}`} />}
              </div>
            ))}
          </div>
        </div>
                ...
                <div>
                  ...
                  {formData.paymentMethod === "qris" && (
                    <div className="bg-gray-100 p-4 rounded-lg">
                      ...
                      <p className="text-black text-xs sm:text-sm mb-4">Please transfer the total amount:</p>
                      <div className="grid grid-cols-1 gap-4">
                        <div className="bg-white rounded-xl shadow p-4 flex flex-col items-center">
                          <img src="/image/qris.JPG" alt="QRIS" className="h-40 w-40 object-contain mb-3" />
                          <div className="text-center">
                  ...
                  {formData.paymentMethod === "bank-transfer" && (
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <p className="text-black text-xs sm:text-sm mb-4">Please transfer the total amount to one of the following bank accounts:</p>
                ...
                <div>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-xs sm:text-sm text-gray-600 space-y-1">
                      ...
                      <p>
                        <strong>Address:</strong> {formData.address}, {formData.city}, {formData.province} {formData.postalCode}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-xs sm:text-sm text-gray-600">
                      {formData.paymentMethod === "bank-transfer" && <p>Bank Transfer</p>}
                      {formData.paymentMethod === "qris" && <p>QRIS</p>}
                      ...
                      {items.map((item) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">Size: {item.size} | Color: {item.color}</p>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 rounded-lg border border-gray-200">
                    <h3 className="font-semibold text-gray-900 mb-3 text-sm sm:text-base">Fraud Alert!</h3>
                    <div className="text-xs sm:text-sm text-gray-600">
                      <p className="mb-2">
                        Verify Account Details: Only transfer funds to our official company account. Avoid sending payments to personal accounts or
                        unverified numbers. Confirm Your Payment: After making a transfer, verify your payment immediately through our official
                        channel: 6285166369467. Note: We are not responsible for any losses arising from transactions conducted outside of our
                        official procedures.
                      </p>
            ...
            <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 sticky top-4">
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={`${item.id}-${item.size}-${item.color}`} className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 border border-gray-100 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className={`font-medium text-xs sm:text-sm line-clamp-2 ${isOutOfStock ? "text-gray-500" : "text-gray-900"}`}>{item.name}</h4>
              ...
              {items.some((item) => item.stock === 0) && (
                <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-red-700 text-xs sm:text-sm font-medium">⚠️ Some items in your cart are out of stock. Please remove them to continue.</p>
                </div>
              )}
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 py-1 rounded-full border text-xs sm:text-sm transition ${
                      shippingOption === "pickup" ? "bg-gray-900 text-white border-gray-900" : "border-gray-300 text-gray-700 hover:border-gray-400"
                    }`}
                  >
                    Pick Up in Store
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-full border text-xs sm:text-sm transition ${
                      shippingOption === "flat" ? "bg-gray-900 text-white border-gray-900" : "border-gray-300 text-gray-700 hover:border-gray-400"
                    }`}
                  >
                    Flat Rate Rp20.000
                  </button>
              ...
              <div className="border-t pt-4 space-y-2">
                ...
                <div className="flex justify-between text-xs sm:text-sm">
                  ...
                  <span className="font-medium">{shippingOption === "pickup" ? "Rp 0" : `Rp ${shippingCost.toLocaleString("id-ID")}`}</span>
                </div>
                ...
                <div className="border-t pt-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-base sm:text-lg text-gray-900">Rp {totalAmount.toLocaleString("id-ID")}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 sm:mt-6 p-3 bg-gray-100 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs sm:text-sm text-gray-800 font-medium">Secure Checkout</span>
                </div>
                <p className="text-xs text-gray-700 mt-1">Your payment information is encrypted and secure</p>
              </div>
              ...
              <div className="mt-3 sm:mt-4 p-3 bg-gray-100 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs sm:text-sm text-gray-800 font-medium">Free Returns</span>
                </div>
                <p className="text-xs text-gray-700 mt-1">30-day return policy on all items</p>
              </div>
